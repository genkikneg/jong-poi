...
APP_NGINX_CONF="$REPO_DIR/docker/nginx/app.conf"
...
grep -Fqx 'upstream jong_poi_reverb {' "$GATEWAY_DIR/nginx/conf.d/jong-poi.conf"
grep -Fqx '    location ^~ /app/ {' "$GATEWAY_DIR/nginx/conf.d/jong-poi.conf"
grep -Fqx '        proxy_pass http://jong_poi_reverb;' "$GATEWAY_DIR/nginx/conf.d/jong-poi.conf"
grep -Fqx '    location ^~ /app/ {' "$APP_NGINX_CONF"
grep -Fqx '        set $reverb_upstream reverb:8080;' "$APP_NGINX_CONF"
grep -Fqx '        proxy_pass http://$reverb_upstream;' "$APP_NGINX_CONF"
grep -Fqx '        proxy_set_header Upgrade $http_upgrade;' "$APP_NGINX_CONF"
...
! grep -Fq "unsafe-inline" "$GATEWAY_DIR/nginx/snippets/security.conf"
