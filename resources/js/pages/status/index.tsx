...
type Game = {
    ...
    session?: { id: number; name: string | null; player_count: number } | null;
};
type Summary = {
    total_points: string;
    total_games: number;
...
const percent = (value: number | null) =>
    value === null ? '—' : `${nf.format(value * 100)}%`;
...
export default function StatusPage({
    ...
    const visibleTrend = useMemo(
        () =>
            trend
                ...
                .reduce<
                    Game[]
                >((games, game) => [...games, { ...game, cumulative_points: Number(games.at(-1)?.cumulative_points ?? 0) + Number(game.points) }], []),
        [trend, gameCount],
    );
    ...
    const conditionLabel = [
        ...
        filters.player_count ? `${filters.player_count}人打ち` : '全ルール',
        opponents.find((item) => item.id === filters.opponent_id)?.name,
    ]
        .filter(Boolean)
        .join('・');
    const chart = useMemo(() => {
        const values = visibleTrend.map((game) =>
            chartMode === 'rank'
                ? game.rank
                : chartMode === 'points'
                  ? Number(game.points)
                  : Number(game.cumulative_points ?? 0),
        );
        if (!values.length)
            return {
                points: '',
        ...
        const dots = values.map((value, index) => ({
            x:
                ...
                (values.length === 1
                    ? (width - left - right) / 2
                    : (index / (values.length - 1)) * (width - left - right)),
            y:
                top +
                ((chartMode === 'rank' ? value - min : max - value) / range) *
                    (height - top - bottom),
        }));
        const tickValues =
            chartMode === 'rank'
        ...
        const tickIndexes = Array.from(
            new Set([
                0,
                Math.floor((values.length - 1) / 4),
                Math.floor((values.length - 1) / 2),
                Math.floor(((values.length - 1) * 3) / 4),
                values.length - 1,
            ]),
        );
    ...
    const primaryMetrics = [
        [
            ...
            summary.average_rank === null
                ? '—'
                : `${nf.format(summary.average_rank)}位`,
        ],
        [
            '平均ポイント',
            summary.average_points === null
                ? '—'
                : `${nf.format(summary.average_points)} pt`,
        ],
    ...
    return (
        <AppLayout breadcrumbs={[{ title: '戦績', href: statusRoute().url }]}>
            <Head title="戦績" />
            <div className="space-y-5">
                <header className="flex items-end justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-semibold">戦績</h1>
                ...
                <Card className="overflow-hidden">
                    <CardContent className="p-0">
                        <div className="grid lg:grid-cols-[1.15fr_2fr]">
                            <div className="flex flex-col justify-center border-b bg-muted/35 p-6 sm:p-8 lg:border-r lg:border-b-0">
                                <p className="text-sm text-muted-foreground">
                                    累計ポイント
                                </p>
                                <p className="mt-1 text-4xl font-bold tracking-tight sm:text-5xl">
                                    {nf.format(Number(summary.total_points))}
                                    <span className="ml-2 text-xl font-medium">
                                        pt
                                    </span>
                                </p>
                                <p className="mt-3 text-sm text-muted-foreground">
                                    {summary.total_sessions}セッション・
                                    {summary.total_games}戦
                                </p>
                            </div>
                    ...
                    <Card className="min-w-0 overflow-hidden">
                        <CardHeader className="pb-2">
                            <div className="space-y-3">
                                        ...
                                        {([10, 30, 50] as const).map(
                                            (count) => (
                                                <button
                                                    ...
                                                    className={`rounded px-3 py-1 text-sm ${desktopGameCount === count ? 'bg-primary text-primary-foreground' : ''}`}
                                                >
                                                    {count}戦
                                                </button>
                                            ),
                                ...
                                <div className="flex w-fit max-w-full rounded-md border p-1">
                                    {(
                                        [
                                            ['rank', '順位'],
                                            ['points', 'ポイント'],
                                        ...
                                        <button
                                            ...
                                            className={`rounded px-2.5 py-1 text-xs sm:px-3 sm:text-sm ${chartMode === value ? 'bg-primary text-primary-foreground' : ''}`}
                                        >
                                            {label}
                                        </button>
                                    ))}
                                ...
                                >
                                    {chart.yTicks.map((tick) => (
                                        <g key={tick.value}>
                                            ...
                                            <text
                                                ...
                                                y={tick.y + 7}
                                                textAnchor="end"
                                                className="fill-foreground text-[19px] font-medium"
                                            >
                                                {chartMode === 'rank'
                                                    ? `${tick.value}位`
                                    ...
                                    {chart.dots.map((point, index) => (
                                        <circle
                                            ...
                                            strokeWidth="3"
                                        >
                                            <title>{`${index + 1}戦目: ${chartMode === 'rank' ? `${visibleTrend[index].rank}位` : `${nf.format(chartMode === 'points' ? Number(visibleTrend[index].points) : Number(visibleTrend[index].cumulative_points))} pt`}`}</title>
                                        </circle>
                                    ))}
                                </svg>
                            )}
                    ...
                    <Card className="min-w-0 overflow-hidden">
                        ...
                        <CardContent className="min-w-0 space-y-3">
                            {[1, 2, 3, 4].map((rank, index) => {
                                const item = detailedStats.rank_rates[rank] ?? {
                                    count: 0,
                                    rate: 0,
                                };
                                return (
                                    <div key={rank}>
                                        <div className="mb-1 flex justify-between text-sm">
                                            <span
                                                className={`font-medium ${rankTextColors[index]}`}
                                            >
                                                {rank}位
                                            </span>
                                            <span>
                                                {item.count}戦{' '}
                                                <strong
                                                    className={`ml-2 ${rankTextColors[index]}`}
                                                >
                                                    {percent(item.rate)}
                                                </strong>
                                            </span>
                                        ...
                                        <div className="h-2 overflow-hidden rounded bg-muted">
                                            <div
                                                className={`h-2 rounded ${rankColors[index]}`}
                                                style={{
                                                    width: `${item.rate * 100}%`,
                                                }}
                                            />
                ...
                <Card>
                    ...
                    <CardContent className="grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-3 lg:grid-cols-6">
                        {[
                            ['半荘最高', detailedStats.best_game_points, 'pt'],
                            ['半荘最低', detailedStats.worst_game_points, 'pt'],
                            [
                                'セッション最高',
                                detailedStats.best_session_points,
                                'pt',
                            ],
                            [
                                'セッション最低',
                                detailedStats.worst_session_points,
                                'pt',
                            ],
                            [
                                'セッション平均',
                                detailedStats.average_session_points,
                                'pt',
                            ],
                            ['飛び率', detailedStats.flying_rate * 100, '%'],
                                ...
                                >
                                    ...
                                    <div className="flex items-baseline gap-5">
                                        ...
                                        <strong className="min-w-20 text-right">
                                            {nf.format(Number(game.points))} pt
                                        </strong>
                                    </div>
                                </div>
                            ))
