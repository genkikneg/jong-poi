...
export default function SessionHistoryPage({
    ...
    const opponent = opponents.find(
        (item) => item.id === filters.opponent_id,
    )?.name;
    const conditionLabel = [
        ...
        .filter(Boolean)
        .join('・');
    const pageItems: (number | 'ellipsis')[] = Array.from(
        new Set([
            1,
            ...Array.from(
                { length: 5 },
                (_, index) => sessions.current_page - 2 + index,
            ).filter((page) => page > 1 && page < sessions.last_page),
            sessions.last_page,
        ]),
    )
    ...
    return (
        <AppLayout
            breadcrumbs={[{ title: '履歴', href: sessionsHistory().url }]}
        >
            <Head title="セッション履歴" />
                ...
                <header className="flex items-end justify-between gap-4">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                            {conditionLabel}・{sessions.total}件
                        </p>
                    </div>
                            ...
                            <div className="grid gap-3 sm:grid-cols-2">
                                ...
                                <label className="space-y-1 text-sm">
                                    ...
                                    <select
                                        ...
                                        onChange={(event) =>
                                            apply({
                                                player_count: event.target.value
                                ...
                                <label className="space-y-1 text-sm sm:col-span-2">
                                    ...
                                    <select
                                        ...
                                        onChange={(event) =>
                                            apply({
                                                opponent_id: event.target.value
                                                    ? Number(event.target.value)
                                                    : null,
                                            })
                                        }
                ...
                {sessions.last_page > 1 && (
                    ...
                    >
                        <Button
                            ...
                            size="sm"
                            onClick={() =>
                                moveToPage(sessions.current_page - 1)
                            }
